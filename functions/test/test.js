...
import {helloWorld, rawBookUploaded} from "../index.js";
...
describe("Customer creation via Firebase Auth", () => {
  ...
  it(`test rawBookUploaded`, async () => {
    const wrapped = firebaseTest.wrap(rawBookUploaded);
    const data = {};
    const result = await wrapped(data, {
      auth: {
        uid: userData.uid,
